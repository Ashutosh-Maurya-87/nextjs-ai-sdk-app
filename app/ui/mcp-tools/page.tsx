...
export default function MCPToolChatPage() {
                                ...
                                {msg.parts.map((part, index) => {
                                    return (() => {
                                        switch (part.type) {
                                            ...
                                            case "dynamic-tool":
                                                return (
                                                    <div key={`${msg?.id}-dynamic-tool-${index}`} className="bg-[#18181b] border border-gray-700 p-4 rounded-xl mt-2">
                                                        {(() => {
                                                            switch (part.state) {
                                                                case "input-streaming":
                                                                    return (
                                                                        <div key={`${msg?.id}-dynamic-tool`}>
                                                                            <div className="text-sm text-blue-400 mb-2">Prepearing Stock Price Request...</div>
                                                                        </div>
                                                                    );
                                                                case "input-available":
                                                                    return (
                                                                        <div key={`${msg?.id}-dynamic-tool-${index}`}>
                                                                            <div className="text-sm text-gray-400">Fetching Stock Price</div>
                                                                        </div>
                                                                    );
                                                                case "output-available":
                                                                    return (
                                                                        <div key={`${msg?.id}-dynamic-${index}`}>
                                                                            <div className="text-sm font-bold text-emerald-400 mb-1">Stock Price Retrived</div>
                                                                        </div>
                                                                    );
                                                                case "output-error":
                                                                    return (
                                                                        <div key={`${msg?.id}-dynamic-${index}-tool`} className="text-red-400 text-sm">
                                                                            <span className="font-bold">Failed to fetch Stock Price:</span> <div>{part.errorText}</div>
                                                                        </div>
                                                                    );
                                                                default:
                                                                    return null;

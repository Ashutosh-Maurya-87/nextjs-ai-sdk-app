...
export async function POST(req: Request) {
    try {
        ...
        const { embeddings: moviewEmbeddings } = await embedMany({
        ...
        const moviewWithScore = movies.map((movie, indx) => {
            const similarity = cosineSimilarity(queryEmbedding, moviewEmbeddings[indx])
            return {
                ...movie,
                similarity
            }
        ...
        moviewWithScore.sort((a, b) => b.similarity - a.similarity)

        const threshold = 0.4
        const relevantRes = moviewWithScore.filter(movie => movie.similarity > threshold)

        const toolResults = relevantRes.slice(0, 3)
        return Response.json({ query, reuslts: toolResults })
    } catch (error: any) {
